...
export class NotaServiceFactory {
  constructor(
    ...
    Logger.debug(
      `Notas disponíveis ${geradoresNotas
        .map((a) => a.identificador().toString())
        .join(',\n')}`,
    );
  }
  ...
  public buildNota(props: { identificador: symbol }): IGeracaoNota {
    ...
    const estrategiaAlvo = this.geradoresNotas.find(
      (gerador) =>
        gerador.identificador().toString() === identificador.toString(),
    );
    if (!estrategiaAlvo)
      throw new Error('Estrategia não registrada como gerador de nota');
