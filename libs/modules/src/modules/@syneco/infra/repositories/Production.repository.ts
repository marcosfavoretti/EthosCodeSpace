  ...
  implements IProductionRepository
{
  ...
  async findProductionByItemClient(
    itens: Array<string>,
  ): Promise<Production[]> {
    ...
    return await this.createQueryBuilder('production')
      ...
      .andWhere('production.PlannedEndTimestamp > :todayMidnight', {
        todayMidnight,
      })
      .andWhere(
        new Brackets((qb) => {
  ...
  async findByPedidoAndDataEntrega({
    partcode,
    pedido,
    dataEntrega,
  }: {
  ...
  async findProductionsStategy(): Promise<Production[]> {
    ...
    return await this.createQueryBuilder('production')
      ...
      .where('production.PartCode LIKE :partCodePattern', {
        partCodePattern: '%-000-%',
      })
      .andWhere('YEAR(production.PlannedEndTimestamp) = :currentYear', {
        currentYear,
