...
export class EstructGradeRepository extends Repository<EstrutGrade> {
  constructor(@InjectDataSource('logix') private dataSource: DataSource) {
  ...
  async getChildren(itemPai: string): Promise<EstruturaOracleChildrenResult[]> {
    ...
    const result = await this.dataSource
      .createQueryBuilder(EstrutGrade, 'eg')
      .select([
        'eg.codItemCompon',
        'eg.qtdNecessaria',
        'item.denItemReduz',
      ...
      .andWhere(
        (qb) => {
          qb.where('(eg.datValidadeIni IS NULL AND eg.datValidadeFim IS NULL)')
            .orWhere(
              '(eg.datValidadeIni IS NULL AND eg.datValidadeFim >= :data_hj)',
            )
            .orWhere(
              '(eg.datValidadeFim IS NULL AND eg.datValidadeIni <= :data_hj)',
            )
            .orWhere(
              '(:data_hj BETWEEN eg.datValidadeIni AND eg.datValidadeFim)',
            );
        },
        { data_hj },
      )
