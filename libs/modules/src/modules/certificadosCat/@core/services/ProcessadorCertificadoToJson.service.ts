...
import { CertificadosOutput, TestGroup, TestStep } from "../class/CertificadoOutput";
...
export class ProcessadorCertificadoToJson {
    ...
    private readonly regexStep = /^\s*(?<category>\w+)\s+(?<status>Pass|OK|Fail)\s+(?<description>.+?)(?:\s+(?<valMeas>-?[\d\.]+)\s*(?:\((?<valRef>-?[\d\.]+)\))?\s+(?<valLimit>[\d\.]+)\s*(?<unit>[A-Za-z]+))?\s+(?<time>\d{1,2}:\d{2}\s+[AP]M)\s*$/;
     ...
     */
    public parseString(content: string, serialNumber: string | undefined = undefined): CertificadosOutput {
        this.reset();
    ...
    private processLine(line: string): void {
        ...
        if (endMatch && endMatch.groups && this.currentGroup) {
            ...
            const hasFailure = this.currentGroup.steps.some(step => step.status === 'Fail');
        ...
        if (this.currentGroup) {
            ...
            if (stepMatch && stepMatch.groups && stepMatch.groups.category === this.currentGroup.name) {
