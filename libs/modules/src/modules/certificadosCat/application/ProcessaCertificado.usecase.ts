    ...
    try {
      ...
      if (certificadoExistente) {
        throw new ConflictException(
          `Certificado com serial number ${serianumber} já existe e não pode ser processado como novo.`,
        );
      }
      ...
      if (!produto) {
        // serialNumber já foi verificado
      ...
      if (error.code === 'ENOENT') {
        throw new BadRequestException(
          'Arquivo não encontrado: ' + dto.filepath,
        );
      }
      if (error.code === 'EACCES') {
        throw new ForbiddenException(
          'Sem permissão para ler o arquivo: ' + dto.filepath,
        );
      }
      ...
      throw new InternalServerErrorException(
        'Erro ao processar o certificado: ' + error.message,
      );
    }
  }
}
