...
export class ExportEstruturaUsecase {
    ...
    constructor(
        ...
        @Inject(LoadEstruturaOracleService) private loaderService: LoadEstruturaOracleService,
    ...
    async export(partcode: ConsultaPorPartcodeReqDTO): Promise<EstruturaDto> {
        try {
            ...
            const estruturaExistente = await this.estruturaRepository.getEstrutura(partcode);
            if (estruturaExistente) await this.estruturaRepository.deleteEstrutura(partcode);

            console.time('relational search')
            const tree = await this.loaderService.generate(partcode);
            console.timeEnd('relational search')

            if (!tree.getChildren().length) throw new BadRequestException('hierarquia com problema');
            if (tree.getStatus() !== 'F') throw new BadRequestException('Estrutura nao é um item final');
            ...
            if (!result) throw new BadRequestException('a estrutura nao foi salva no banco de dados');
            return result;
        } catch (error) {
            console.error(error);
