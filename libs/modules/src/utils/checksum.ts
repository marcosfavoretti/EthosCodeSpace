const preCalcTable: number[] = [
  ...
  0xed, 0xea, 0xb7, 0xbe, 0xb9, 0xbe, 0xab, 0xac, 0xa5, 0xa2, 0x8f, 0x88, 0x81,
  ...
  0x87, 0x80, 0x95, 0x92, 0x9b, 0x9c, 0x81, 0xb6, 0xbf, 0xb8, 0xad, 0xaa, 0xa3,
  0xa4, 0xf9, 0xfe, 0xf7, 0xf0, 0xe5, 0xe2, 0xeb, 0xec, 0xc1, 0xc6, 0xcf, 0xc8,
  0xdd, 0xda, 0x03, 0xd4, 0x69, 0x6e, 0x67, 0x60, 0x75, 0x72, 0x7b, 0x7c, 0x51,
  ...
  0x3e, 0x39, 0x30, 0x37, 0x22, 0x25, 0x2c, 0x28, 0x06, 0x01, 0x08, 0x0f, 0x1a,
...
export function generateChecksum(props: { data: string }): {
  data: string;
  result: string;
  checksum: string;
} {
  let crc = 0x00;
  props.data.split('').forEach((_, idx) => {
    crc ^= props.data.charCodeAt(idx);
    crc = preCalcTable[crc];
  });
  const checksum = crc.toString(16).padStart(2, '0').toUpperCase();
  return {
    checksum,
    data: props.data,
    result: props.data.concat(checksum),
  };
}
