...
export class DirWatcherService implements OnModuleInit, OnModuleDestroy {
  ...
  setupTriggers(): void {
    const dirPath = this.configService.get<string>(
      'CERTIFICADOS_CAT_WATCHER_DIR_PATH',
    );

    const rawIgnora = this.configService.get<number>(
      'IGNORA_CERTIFICADOS_INICIAIS',
      1,
    );
    ...
    if (!dirPath) {
      this.logger.error(
        'VARIAVEL CERTIFICADOS_CAT_WATCHER_DIR_PATH NÃO DEFINIDA.',
      );
      return;
    }
    ...
    this.watcher.on('add', (filePath) =>
      this.handleFileEvent(filePath, 'adicionado'),
    );
    this.watcher.on('change', (filePath) =>
      this.handleFileEvent(filePath, 'modificado'),
    );
  ...
  private async handleFileEvent(
    filePath: string,
    eventType: 'adicionado' | 'modificado',
  ) {
    ...
    try {
      ...
      this.logger.error(
        `Erro crítico ao processar ${fullFileName}`,
  ...
  private async handleModificationSafely(filePath: string) {
    try {
      ...
      if (error instanceof CertificadoNaoCriadoException) {
        this.logger.warn(
          `Tentativa de modificação em arquivo não indexado. Criando agora: ${path.basename(filePath)}`,
        );
        await this.processaCertificadoUseCase.execute({ filepath: filePath });
      } else {
        throw error;
