...
import { __NotaAlmox } from '@app/modules/modules/notas/@core/service/AlmoxNota.service';

@Controller('notas')
export class NotasOrquestradasController {

  constructor(
    @Inject(__NOTAS_QUEUE) private readonly client: ClientProxy,
  ) { }

  @ApiOperation({
    summary:
      `Cria os seguintes tipos de notas : ${[__NotaAlmox.toString(), __NOTAS_QUEUE.toString()].join(', ')}`
  })
  @Post('sync')
  async generatePdf(
    ...
    try {
      ...
      const pdfsResponse = await Promise.all(
        chunks.map(chunk =>
          firstValueFrom(
            this.client.send<GerarNotaResDTO, CreateNotaReqDTO>({ cmd: NOTA_CRIACAO }, {
              payload: chunk,
              tipo: tipo
            })),
        ),
